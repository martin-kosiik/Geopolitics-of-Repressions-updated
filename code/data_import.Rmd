...
data_labeled <- data_labeled %>% 
  mutate_at(vars(ethnicity, surname, name, patronimic), as.factor)


nb_klaR <- NaiveBayes(ethnicity ~ surname + name + patronimic, data = data_labeled, 
                             fL = 1, usekernel = FALSE)

data_labeled$prediction <- predict(nb_klaR)$class
```
