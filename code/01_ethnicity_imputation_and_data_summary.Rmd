...
```{r}
memorial_lists %>% 
  mutate(no_rehab = ifelse(rehabilitation_date == "None", 1, 0)) %>% 
  count(no_rehab, no_date, sort = T) %>% 
  View()

memorial_lists %>% 
  mutate(no_rehab = ifelse(rehabilitation_date == "None", 1, 0)) %>% 
  count(no_rehab, sort = T) 

count_places <- memorial_lists[, .(count = .N), by = live_place][order(-count)]
count_birth <-  memorial_lists[, .(count = .N), by = birth_year_comment][order(-count)]
```
