...
min_by_q <-  min_by_month %>% 
  ...
  group_by(ethnicity) %>% 
  mutate(t = row_number(), 
         pre_treatment = ifelse(year_q_date >= "1928-01-01" & year_q_date <= "1933-01-01", 1, 0),
...
post_yq_dummies <- map_dfc(year_quarters, ~  if (dplyr::last(year_quarters) == .){
...
diff_model_robust <- commarobust(diff_model, clusters = min_by_q$ethnicity, se_type = "stata")
diff_model_window_robust <- commarobust(diff_model, clusters = min_by_q$ethnicity, se_type = "stata")
...
ggsave(here::here("plots/effects/quarterly/pointrange.pdf"))
