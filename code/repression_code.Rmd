...
stargazer(diff_model, diff_model_restr, keep = "german:post_+", omit.stat = c("ser", "f"), label = "dif_table",
          covariate.labels = var_labels, title = "Difference-in-difference results",
          dep.var.labels = "$\\log(1 + y_{it})$", 
          add.lines = list(c("Year dummies", "Yes", "Yes"),
                           c("Ethnicity dummies", "Yes", "Yes")),
          column.labels = c("1923-1958", "1923-1958")) %>% 
...
diff_model %>% 
  ...
  labs(x = "year", title = "Estimates of interaction of german and (lagged and leaded) post variables")
