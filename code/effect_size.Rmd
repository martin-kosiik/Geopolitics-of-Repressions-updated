...
arrests_in_year <- function(base_year = 1921){
  totals %>% 
...
pct_effects <- function(formula = fmla_pred_full_imp_date_no_trends_geopol, data = min_by_year, base_year = 1921){
...
pct_effects(fmla_pred_full_imp_date_no_trends_geopol_from_1933, base_year = 1932) %>% 
  View()
...
total_effects_synth <- function(data, year_from = 1932, year_til = 1961){
  data %>% 
    ...
    summarize_at(c("effect_count", "count_synth", "count_treat"), sum) %>% 
    as_tibble()
}
