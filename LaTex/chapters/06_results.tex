...
% most of them are not significantly different from zero
 ...
 In any case,  the impact of war on the repressions appears to be highly persistent as 
...
To convert these coefficients to percentage changes, we apply formula \ref{eq:dummy_vars}. Since the base of our comparison is very small (only 135 of arrests of Germans in 1921 compared to tens of thousands of arrests during war), the calculated percentage changes are extremely large. 
...
As we already hinted, the pre-1933 coefficients in figure \ref{fig:did_effets} give us a reason to doubt the
...
Another potential issue is that some ethnic groups might have changed their treatment status in this period in various complicated ways. 
...
the estimated effect for German ethnicity  still appears relatively large for the period from 1939 to 1955, although for years 1933-1938 the effect for Germans fits within the standard range of the placebo gaps.

Figure \ref{fig:sc_placebo_gaps_all} also shows that for some ethnic groups the pre-treatment gaps are fairly large.
The most extreme example is the gap for Russians which stays above 2 log points for the  whole pre-1933 period. 
...
The resulting plot is shown in figure \ref{fig:sc_placebo_gaps_all_20_times}. The post-1933 gaps in German arrests now stand out little more clearly. %and even the gap for the years 1933-1938 now appears relatively significant.  
...
The MSPE ratio for German minority is now only the fourth highest and therefore the implied $p$-value  is  4/38 ($\approx 0.105$) which would no longer be considered significant at 10\% level.
...
In the Memorial database, there  are 1 257 796 individuals  classified as  rehabilitated (for rest of the data we either do not have the information on rehabilitation or they were not eligible for rehabilitation).  We thus re-estimate our baseline specification using only those observations. The results are provided in table \ref{dif_table} in the column (3) and plotted in figure \ref{fig:did_effects_rehabs}. We see that the coefficients after 1933 change only little. Nonetheless, the pre-treatment estimates deviate from zero more than in previous specifications.
%this is a consequence of h

Second, we explore if there is heterogeneity in the effect by border areas.
\citet{mcnamee_demographic_2019} found that the expulsions of minorities with ethnic ties to hostile power took place in border areas without natural barrier.
They explain that this is motivated by state's effort to   consolidate its power in a territory that is  vulnerable to foreign invasion.  
The question is if similar logic also applies for repressions as well.\footnote{We do not consider distinction between borders with and without natural barriers since most of Soviet border frontiers in Europe lack any such a barrier. } 
%This is motivated by a hypothesis of \citet{mcnamee_demographic_2019} who claim that states are more likely to expel ethnic minorities in border areas in order to  prevent cross-border insurgencies and consolidate power in 
...
We see in figure \ref{fig:did_effects_non_border} that the dynamics and size of the effect for non-border regions is very similar to our previous specifications
%and the size of the coefficients do not appear to be diminished at all. 
The border regions, on the other hand, seem to experienced much greater increase in arrests of Germans in the years 1934-1939 (results are shown in figure \ref{fig:did_effects_border}). But the pre-1933 coefficients have much larger deviations from zero which casts stronger doubt on the parallel trends assumptions and credibility of the results. 
Thus, we cannot draw strong conclusions based on these results. 
%Thus, we cannot make strong conclusions with regard to the heterogeneity of the effect by border area.
%we are mostly inconclusive with regard to the heterogeneity by border area.
%Nevertheless, the for the years 1924 and 1925, the pre-treatment trends deviate from zero  
...
We see that the estimates for the full matrix (used by default) and the parsimonious adjustment are virtually the same. When no adjustment is applied, the coefficients get only slightly smaller. 
...
When we compare the gaps between synthetic control and actual value (figure \ref{fig:sc_placebo_gaps_robustness}) with our baseline model, we notice three main differences. First, pre-treatment fit for Germany is substantially worse which is  the cost of
...
Third, we apply again the same baseline synthetic control method but only to data with rehabilitated individuals. The estimated $W$-weights are provided in table \ref{tab:sc_weights_rehabs}. The trends in the gaps between the actual and synthetic repressions (plotted in figure  \ref{fig:sc_placebo_gaps_all_rehabs}) are broadly consistent with  our previous estimates. 
...
Finally, we test if there are different patterns in the repressions  in the border frontiers. We see in figure \ref{fig:sc_placebo_gaps_by_border_comp} that the post-1933 gaps in repressions in both border and non-border areas follow very similar overall trends (sharp surge with war persisting until 1955). Nonetheless, the effect size in border areas seems slightly larger. 
Furthermore, the pre-treatment fit of the synthetic control in the border regions is somewhat better. As a consequence, the $p$-value for
border regions is 1/38 but only 4/38 ($\approx 0.105$) for non-border areas.
%non-border areas is only 4/38 ($\approx 0.105$) and 1/38 for border areas. 
Thus, there is some evidence that increase in repressions  was higher in the border frontiers which is consistent with the results of \citet{mcnamee_demographic_2019}. 

%However, there might be other explanations for these results as well (e.g. the German dias
%This would be consistent with the hypothesis of \citet{mcnamee_demographic_2019} who argue that states use repressions as a way to secure their vulurable. However, there might other explanations for these results as well (e.g. that there was greater German diaspora in border regions) and therefore we cannot make definitive conclusions. 

%the MSPE ratio (shown in figure \ref{fig:sc_mspe_ratios_by_border_comp})
%The results \ref{fig:sc_placebo_gaps_by_border_comp} and\ref{fig:sc_mspe_ratios_by_border_comp}
