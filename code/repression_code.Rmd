...
min_by_year <-  read_csv(here::here("data/min_by_year_preds.csv")) %>% 
  ...
  mutate(german = ifelse(ethnicity == "German", 1, 0),
         ...
         YEAR_sq = YEAR^2, 
         pre_treatment = ifelse(YEAR >= 1930 & YEAR < 1933, 1, 0),
...
min_by_year %>% 
  group_by(YEAR) %>% 
  summarise(pred_label = sum(label_imp_date) + sum(prediction_imp_date)) %>% 
  View()
...
ggsave(here::here("plots/effects/ethnicity_imputation/annual/pr_cr2_date_imp_full_years_ind_country.pdf"))
 ...
 min_by_year %>% 
  ...
  plot_effects_robust_se(formula = fmla_pred_full_imp_date)
 ggsave(here::here("plots/effects/ethnicity_imputation/annual/pr_cr2_date_imp_full_years_not_ind_country.pdf"))
