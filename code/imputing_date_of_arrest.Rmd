...
missing_date_of_arrest %>% 
  ...
  ggplot(aes(x = diff_day_imp_complex)) + geom_histogram(col = "white") + theme_minimal()+
  scale_x_continuous(trans = "mylog10")
...
both_dates %>% 
  ...
  ggplot(aes(x = diff_days)) + geom_histogram(aes(y=..density..), col = "white") + 
    ...
    stat_function(fun = function(x, mean, sd) dnorm(log(x + 1) , mean, sd), 
                  args = list(mean = lm_model$coefficients[1], sd = summary(lm_model)$sigma), 
                  col = "blue", size=1.5) + scale_x_continuous(trans = "mylog10") +
...
lm_model <-  lm(log(1 +diff_days) ~ 1, data = both_dates)
