...
After imputations of ethnicity and the date of arrest had been applied,  we created our main dataset by counting number of arrest for each ethnicity by year. In the original dataset, there were more than 120 different ethnic groups howeve
...
\subsection{Imputing Missing Date of Arrest} \label{subsec:imputing_missing_date}
 We show the histogram for number of days between arrest and process (on scale $\log(1 + x)$) in the figure \ref{fig:simple_lm_hist}. First, we can see that there is fairly large variance in the variable with majority of number of days ranging from 8 to 1000.  However, the transformed data seems to be following normal distribution.  
...
\end{equation}
After  these models are fitted to the data with non-missing date of arrest and year of process, we apply them to the missing data to get the predicted probability of $y$  being non-zero and the mean value of $y$ if it is non-zero. For each observation with missing date of arrest, we then randomly draw from the Bernoulli distribution with the predicted probability as its parameter to obtain $\hat I_i^y$. We also draw  obtain
