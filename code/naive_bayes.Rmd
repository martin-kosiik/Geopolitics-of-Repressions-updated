...
train_data <- filter(events, !is.na(ethnicity), prediction == 0)
...
test$prediction <- predict(nb_klaR, test)$class
conf_matrix <- confusionMatrix(test$prediction, test$ethnicity)
conf_matrix$byClass
conf_matrix$positive
