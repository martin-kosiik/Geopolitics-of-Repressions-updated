...
sc_results$combined %>% 
  ...
  filter(variable %in% c("log_n","pop_total", "clad_sim", "urb_rate")) %>% 
  filter(index < 1933) %>% 
 # View()
  group_by(variable) %>% 
  summarize_all(mean) %>% 
...
mspe_ratios <-  ppratio(sc_placebo, "log_n", range.pre = c("1921","1932"),
...
ggsave(here::here("plots/synthetic_control/annual/mspe_ratios.pdf"))
