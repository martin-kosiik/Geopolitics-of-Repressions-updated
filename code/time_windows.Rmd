...
## Position dodge
map(trends_formulas_full_years, ~ fit_robust_model(formula = ., vcov = "CR0", estimatr = 1)) %>% 
  bind_rows(.id = "id") %>% 
...
## Position dodge
map(pred_adj_formulas_full_years, ~ fit_robust_model(formula = ., vcov = "CR2", estimatr = 0)) %>% 
  bind_rows(.id = "id") %>% 
  mutate(nice_labs = str_replace(term,"german:year_",""),
         nice_labs = ifelse(as.numeric(nice_labs) == max(as.numeric(nice_labs)), str_c(nice_labs, "+"), nice_labs)) %>% 
