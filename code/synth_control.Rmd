...
ethnicity_controls %>% 
  ...
  kable("latex", align = "c", booktabs = T, digits = 2, linesep = "", format.args = list(big.mark = " "),
        col.names = c("Ethnic group", "Total population", "Linguistic similarity to Russian", "Urbanization rate"),
        caption = "Pre-treatment characterisitcs of ethnic groups in the USSR") %>%
  footnote(general = "Total population and urbanization rate of the ethnic group in the USSR is taken from 1926 census. 
           The linguistic similarity to Russian is measured by the number of common nodes in the language tree (cladistic similarity).",
           threeparttable = T) %>% 
...
placebo
```{r}
sc_placebo <- mscmt(data_prep_mscmt, treatment.identifier, controls.identifier, times.dep, times.pred, agg.fns, seed=2019, placebo = TRUE)
...
tibble(mspe_ratios, ethnicity = names(mspe_ratios)) %>% 
  mutate(German = as.factor(ifelse(ethnicity == "German", 1, 0))) %>% 
  ggplot(aes(x = mspe_ratios, fill = German)) + geom_histogram() + theme_bw() +
  labs(x = "MSPE ratio", title = "Ratios of post to pre-treatment period mean squared prediction error")
