...
```{r}
memorial_lists %>% 
  count(criminal_article, sort = T) %>% 
  mutate(kulak = str_detect(criminal_article, "кулаки")) %>% 
  View()

kulaks <- memorial_lists %>% 
  filter(criminal_article %in% c("кулаки (Постановление СНК и ЦИК СССР от 1.02.1930)", 
                                 "как кулаки", "Кулаки"))
head(memorial_lists)
...
kulaks <- memorial_lists %>% 
  filter(str_detect(criminal_article, pattern =  fixed("Кулак", ignore_case = TRUE)))

memorial_lists %>% 
  count(rehabilitation_reason, sort = T) %>% 
  View()
...
selected_vars <- c("person_id", "nation", "arest_date", "process_date","rehabilitation_reason",
                   "criminal_case", "criminal_article", "sentence", "rehabilitation_date",
...
memorial_lists %>% 
  ...
  add_header_above(c(" " = 1, "Date of Arrest or Process" = 2)) %>% 
...
```{r}
data_labeled <- droplevels.data.frame(data_labeled)

rand <-  sample(1:2, size = nrow(data_labeled), replace = TRUE)
train_set <- data_labeled[rand == 1,]



nb_model <- naive.bayes(train_set, "ethnicity")
pred = predict(nb_model, data_labeled, prob = TRUE)
head(t(attr(pred, "prob")))

pred_probs <- as_tibble(t(attr(pred, "prob")))

pred_probs %>% 
  gather(key = "ethnicity", value = "probabilty") %>% 
  ggplot(aes(x = probabilty)) + geom_histogram() + facet_wrap(~ ethnicity)

pred_probs %>% 
  ggplot(aes(x = Bashkir)) + geom_histogram()

 confusionMatrix(pred, data_labeled$ethnicity)
