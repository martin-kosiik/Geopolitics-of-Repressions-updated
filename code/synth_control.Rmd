...
ethnicity_controls %>% 
  ...
  kable("latex", booktabs = T, digits = 2, linesep = "", format.args = list(big.mark = " "),
        col.names = c("Ethnic group", "Total population", "Ling. similarity to Russian", 
                      "Urbanization rate", "Ind. country"),
        caption = "Pre-treatment characteristics of ethnic groups in the USSR") %>%
  footnote(general = "Total population and urbanization rate of the ethnic group in the USSR is taken from 1926 census. The linguistic similarity to Russian is measured by the number of common nodes in the language tree (cladistic similarity). Independent country equals one if the ethnic group was a core group in an independent state that existed in the interwar period.",
           threeparttable = T) %>% 
...
times.dep  <- cbind(dep_var                 = c("1921","1933"))
...
dep_var <- "log_n_pred_full"
times.dep  <- cbind("log_n_pred_full" = c("1921","1933"))
...
times.dep  <- cbind("log_n_pred_full_imp_date" = c("1921","1933"))
...
sc_results$combined %>% 
  ...
  filter(variable %in% c(dep_var,"pop_total", "clad_sim", "urb_rate")) %>% 
  filter(index < 1933) %>% 
 # View()
  group_by(variable) %>% 
  summarize_all(mean) %>% 
 ...
 tibble(w = sc_results$w, ethnicity = names(sc_results$w)) %>% 
   left_join(ethnicity_controls, by = "ethnicity") %>% 
   mutate_at(vars(clad_sim, urb_rate, pop_total), ~ sc_results$w * .) %>% 
   summarise_at(vars(clad_sim, urb_rate, pop_total), ~ sum( .))
    
sc_results$combined %>% 
  keep(~ names(.) %in% c("log_n","pop_total", "clad_sim", "urb_rate"))
...
placebo_highlight_mspe <- function(placebo_data = sc_placebo, dep_var = dep_var){
...
placebo_highlight_mspe(sc_placebo_pred_full_imp_date, "log_n_pred_full_imp_date")
ggsave(here::here("plots/synthetic_control/ethnicity_imputation/annual/placebo_highlight_mspe_5_lower_imp_date.pdf"))
...
placebo_mspe_barplot <- function(placebo_data = sc_placebo, dep_var = dep_var){
  

mspe_ratios <-  ppratio(placebo_data, dep_var, range.pre = c("1921","1932"),
                        range.post = c("1933","1939"), type = "mspe")
...
placebo_mspe_barplot(sc_placebo_pred_full, "log_n_pred_full")
...
tibble(ethnicity = names(sc_results$w), Weights = sc_results$w) %>% 
  ...
  kable("latex", booktabs = T, digits = 2, linesep = "", format.args = list(big.mark = " "),
        col.names = c("Ethnic group", "$W$-Weight"),
