...
Finally, we acquired data that would allow us to test for heterogeneity in the effect by proximity to the border. 
...
We defined border frontier as an area that is within 250 kilometers of the Soviet outer borders (they are depicted on the map in figure \ref{fig:map}). 
...
%Furthermore, 147 970 of observations fell outside of the 1926 borders of the Soviet Union. Given the fact that the USSR expanded its territory, some of those cases could be. Nevertheless, we dropped these observations from the analysis. 
